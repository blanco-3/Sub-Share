...
const T = {
  en: {
    appName: "Sub-Share",
    tagline: "Reimburse shared Claude bills without chasing teammates.",
    taglineSub: "Built for hackathon teams sharing Claude. Everyone locks their share upfront, and the teammate who paid can recover it through approvals or a matching zkTLS proof.",
    ...
    steps: [
      { icon: "01", title: "Create a Claude vault", desc: "Start with the Claude plan one teammate is paying for." },
      { icon: "02", title: "Invite teammates", desc: "Share the vault link so the rest of the team can join with social login or wallet." },
      { icon: "03", title: "Lock reimbursement upfront", desc: "Each teammate deposits their share before the payer asks for anything back." },
      { icon: "04", title: "Reimburse the payer", desc: "The teammate who paid Claude claims the monthly amount through approvals or a matching zkTLS proof." },
    ],
    keyFeature: "No awkward reimbursement chase",
    keyFeatureDesc: "Instead of sending reminders in Discord, teammates pre-commit funds. The payer can only recover the exact monthly reimbursement through contract rules.",
    getStarted: "Start Claude Demo",
    
    // Create
    selectService: "Which bill are you reimbursing?",
    selectServiceDesc: "Claude is preselected for this demo. You can still switch plans or enter a custom bill.",
    perTeam: "/mo per team",
    ...
    reviewDesc: "Review the reimbursement vault before deploying on-chain.",
    monthlyCost: "Monthly subscription",
    members: "Members",
    perPerson: "Per person / month",
    durationLabel: "Commitment",
    depositRequired: "Deposit per person",
    howSafe: "How reimbursement stays honest",
    howSafeDesc: "Members lock USDC upfront. The payer can only claim the monthly amount through unanimous approval, the grace fallback, or a matching Reclaim proof.",
    deployBtn: "Deploy Vault",
    deploying: "Deploying on-chain...",
    
    // Invite
    inviteTitle: "Invite your team",
    inviteDesc: "Share this link so teammates can lock their reimbursement share upfront.",
    ...
    depositDesc: "Each teammate locks their share before the payer claims reimbursement.",
    ...
    paid: "Paid out",
    remaining: "Remaining",
    schedule: "Reimbursement history",
    noPayments: "When the payer covers this month's Claude invoice, the vault releases reimbursement after unanimous approval or a matching zkTLS proof.",
    releaseBtn: "Approve month {n} reimbursement",
    releasing: "Submitting approval...",
    paymentTo: "→ Reimbursed to payer",
    approvedLabel: "{a}/{t} members approved",
    alreadyVoted: "✓ You voted this month",
    proveAndClaim: "Auto-Claim with Stripe Receipt (zkTLS)",
    provingSubscription: "Starting zkTLS proof flow...",
    submittingProofClaim: "Submitting auto-claim...",
    scanQr: "Scan with Reclaim app to prove the Stripe receipt",
    proofVerified: "✓ Proof verified on-chain",
    reimbursementGuide: "How monthly reimbursement works",
    reimbursementDesc: "The vault does not pay Claude directly. The payer covers the invoice first, then pulls the matching monthly reimbursement from funds the team already locked.",
    reimbursementStep1: "Team deposits upfront in USDC",
    reimbursementStep2: "Payer covers the Claude invoice offchain",
    reimbursementStep3: "Claim by team approvals or a matching zkTLS proof",
    reimbursementHint: "zkTLS is a claim acceleration path, not a separate product.",
    
    // Complete
    complete: "Vault schedule complete",
    completeDesc: "All scheduled reimbursements were executed successfully.",
  ...
  ko: {
    appName: "Sub-Share",
    tagline: "Claude 선결제분을 어색한 독촉 없이 정산하세요.",
    taglineSub: "해커톤 팀이 Claude를 함께 쓸 때를 위한 정산 볼트입니다. 모두가 먼저 자기 몫을 잠그고, 선결제자는 승인이나 zkTLS 증명으로 월별 환급을 받습니다.",
    ...
    onboardTitle: "작동 방식",
    steps: [
      { icon: "01", title: "Claude 볼트 생성", desc: "누군가 실제로 결제할 Claude 요금제를 기준으로 시작합니다." },
      { icon: "02", title: "팀원 초대", desc: "링크를 공유하면 다른 팀원이 소셜 로그인이나 지갑으로 참여할 수 있습니다." },
      { icon: "03", title: "정산금 선입금", desc: "선결제자에게 나중에 갚을 금액을 각자 먼저 잠급니다." },
      { icon: "04", title: "선결제자 환급", desc: "Claude를 실제로 결제한 사람이 승인 또는 zkTLS 증명으로 월별 환급을 받습니다." },
    ],
    keyFeature: "정산 독촉을 구조적으로 줄입니다",
    keyFeatureDesc: "디스코드에서 계속 돈 보내달라고 말할 필요 없이, 팀원이 먼저 자기 몫을 커밋합니다. 선결제자는 컨트랙트 규칙에 따라 정확한 월별 환급만 받을 수 있습니다.",
    getStarted: "Claude 데모 시작",
    
    selectService: "어떤 청구서를 정산하나요?",
    selectServiceDesc: "이 데모는 Claude 중심입니다. 다른 플랜이나 커스텀 금액도 선택할 수 있습니다.",
    perTeam: "/월 (팀 기준)",
    ...
    commitment: "계약 기간",
    reviewTitle: "확인 및 배포",
    reviewDesc: "온체인 배포 전에 정산 볼트 설정을 확인하세요.",
    monthlyCost: "월 구독료",
    members: "인원",
    perPerson: "1인당 / 월",
    durationLabel: "계약 기간",
    depositRequired: "1인당 선입금",
    howSafe: "환급이 정직하게 유지되는 이유",
    howSafeDesc: "팀원은 USDC를 먼저 잠급니다. 선결제자는 만장일치 승인, grace fallback, 또는 일치하는 Reclaim 증명이 있을 때만 월별 금액을 청구할 수 있습니다.",
    deployBtn: "볼트 배포",
    deploying: "온체인 배포 중...",
    
    inviteTitle: "팀원 초대",
    inviteDesc: "이 링크를 공유해 팀원이 정산 몫을 먼저 잠그게 하세요.",
    vaultLink: "볼트 초대 링크",
    copied: "✓ 복사됨!",
    copyLink: "링크 복사",
    shareDiscord: "Discord",
    ...
    depositDesc: "선결제자가 환급을 청구하기 전에 팀원 모두가 자기 몫을 먼저 잠급니다.",
    yourShare: "내 입금액",
    ...
    remaining: "잔여",
    schedule: "환급 내역",
    noPayments: "선결제자가 이번 달 Claude 청구서를 결제하면, 만장일치 승인 또는 일치하는 zkTLS 증명으로 환급이 실행됩니다.",
    releaseBtn: "{n}개월차 환급 승인",
    releasing: "승인 제출 중...",
    paymentTo: "→ 선결제자에게 환급됨",
    approvedLabel: "{a}/{t}명 승인",
    alreadyVoted: "✓ 이번 달 투표 완료",
    proveAndClaim: "Stripe 영수증으로 자동 청구 (zkTLS)",
    provingSubscription: "zkTLS 증명 흐름 시작 중...",
    submittingProofClaim: "자동 청구 트랜잭션 제출 중...",
    scanQr: "Reclaim 앱으로 QR 스캔 → Stripe 영수증 증명",
    proofVerified: "✓ 온체인 증명 검증 완료",
    reimbursementGuide: "월별 환급 방식",
    reimbursementDesc: "볼트가 Claude를 직접 결제하지는 않습니다. 선결제자가 먼저 결제하고, 팀이 잠가 둔 자금에서 정확한 월별 금액을 환급받습니다.",
    reimbursementStep1: "팀원 모두 USDC를 먼저 입금",
    reimbursementStep2: "선결제자가 Claude 청구서를 오프체인 결제",
    reimbursementStep3: "팀 승인 또는 일치하는 zkTLS 증명으로 환급 청구",
    reimbursementHint: "zkTLS는 별도 제품이 아니라 환급을 앞당기는 청구 경로입니다.",
    
    complete: "볼트 일정 완료",
    completeDesc: "예정된 모든 환급이 성공적으로 실행되었습니다.",
    newVault: "새 볼트 만들기",
